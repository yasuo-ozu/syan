...
(usage pre-pass + view dispatch, drop `edit_apply`), `core/tests/visitor_edit.rs` (rewrite to the
view API), `CLAUDE.md` bullet. Net: the per-node
...
2. **`Punctuated` inserts** (`Sep: Default`) — shipped; **nested-container** views — shipped (`#[seq]`/
   ...
   `ui/visitor_edit_marker_boxed.rs`).
...
- **View backend — DECIDED: Design B (trait on the containers), IMPLEMENTED.** `SeqView<T>`/`OptView<T>`
  ...
  Shipped on branch `feat-visitor-edit-view` (replaces the v1 `Edit<T>` return); tests
  `visitor_edit.rs`.
