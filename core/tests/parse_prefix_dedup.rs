...
fn three_variant_chain() {
    ...
    assert!(matches!(Chain::parse(quote! { 5 true }).unwrap(), Chain::Two(_, _)));
    assert!(matches!(Chain::parse(quote! { 5 true 6 }).unwrap(), Chain::Three(_, _, _)));
}
...
fn divergent_suffixes() {
    assert!(matches!(Div::parse(quote! { 5 true }).unwrap(), Div::Flag(_, _)));
    assert!(matches!(Div::parse(quote! { 5 6 }).unwrap(), Div::Pair(_, _)));
    assert!(Div::parse(quote! { 5 }).is_err(), "neither suffix matches → error");
}
...
fn named_field_prefix() {
    assert!(matches!(Named::parse(quote! { 5 true }).unwrap(), Named::Tagged { .. }));
    assert!(matches!(Named::parse(quote! { 5 }).unwrap(), Named::Plain { .. }));
}
...
fn prefix_sharing_recurse_cycle() {
    ...
    assert!(matches!(Expr::<()>::parse(quote! { 5 }).unwrap(), Expr::Lit(..)));
...
fn shared_prefix_parsed_once() {
    ...
    assert_eq!(n, 1, "prefix parsed once for `5` (no re-parse on the Tagged→Plain fallback)");
