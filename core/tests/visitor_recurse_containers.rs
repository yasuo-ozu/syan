//! `#[recurse(visit)]` traversal through container shapes and tuples — regression tests for the
//! fixes to audit problems #3 (a `Box` *around* an `Option`, via `Peeled::cont_box`) and #7 (a
//! tuple-typed field, destructured and dispatched element-by-element), plus the already-working
//! `Vec<Box<_>>` / `Option<Box<_>>` shapes for good measure.
//!
//! Back-edge slots are filled with `ast::ExprNode::Lit(PhantomData)` (the depth-generic node alias),
...
fn box_around_option_some() {
    ...
    let e: ast::Expr<()> =
        ast::Expr::Opt(Box::new(Some(Box::new(ast::ExprNode::Lit(PhantomData)))));
...
fn tuple_field_both_operands() {
    ...
    let e: ast::Expr<()> = ast::Expr::Pair((
        Box::new(ast::ExprNode::Lit(PhantomData)),
        Box::new(ast::ExprNode::Lit(PhantomData)),
    ));
...
fn tuple_field_with_leaf() {
    ...
    let e: ast::Expr<()> =
        ast::Expr::Tagged((Box::new(ast::ExprNode::Lit(PhantomData)), PhantomData));
...
fn option_of_boxed_expr() {
    let e: ast::Expr<()> = ast::Expr::OptIn(Some(Box::new(ast::ExprNode::Lit(PhantomData))));
