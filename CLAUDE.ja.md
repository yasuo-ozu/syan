...
- **`#[recurse(visit)]`**（実装済み）: `#[recurse]` の*循環*に対する**深さジェネリック**ビジター。
  ...
  `Box` も含む、`cont_box` 経由）と**タプル**（各要素をディスパッチ）を辿る。循環型は**ライフタイム / 型 /
  const ジェネリックパラメータ**を持てて、循環内の型ごとに*異なる*パラメータでもよい（ヘテロ）: 各型は自分の
  パラメータを保ち（自身の `__*Rec` ノードと公開エイリアスに通す）、`Visit`/`VisitRec` トレイトはルートの
  パラメータでキーされ、型の余分なパラメータはその `visit_*` メソッドのジェネリックになる。唯一の要件は、
  全循環型がルートのパラメータを宣言すること（`__Rec` のデフォルト `__RootDefault<ルートのパラメータ>` を
  綴れるように）。非対応の形は明確な `abort!` で却下する（分かりにくい生成コードエラーではなく）: 入れ子
  コンテナ（`Vec<Option<_>>`）、ルートのパラメータを欠く循環型、多ルート循環。テスト:
  `visitor_recurse_cycle.rs`（ルート / 横断辺 / 後退辺 / 自己再帰ルート）、`visitor_recurse_containers.rs`
  （コンテナ＋タプルの走査）、`recurse_generics.rs`（ライフタイム / 型 / const / ヘテロのパラメータ）、
  `recurse_audit_test.rs` ＋ `ui/recurse_*.rs`（却下；`limit = 0` は今もパニック）。
