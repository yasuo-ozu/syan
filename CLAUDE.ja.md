...
- **`#[subast(<path> [as Alias])]` ＋ drill-in**（実装済み）: `#[derive(Ast)]` は型レベルの `#[subast]`
  ...
  **未列挙の中間型**は**インラインでドリルスルー**され（その定義を分解し、*その* `#[subast]` フィールドへ
  ...
  `visitor_drill*.rs`。（下記「Drill-in — 実装どおり」参照。）
...
> - **パス解決。** `#[subast]` のパスはそのまま出力する；正準な `crate::` 根付けパスを推奨（ビジター
>   モジュールから同一クレートで解決する）。上流クレートの `#[subast]` 型を**下流**で構築するビジター向けの
>   `$crate` 根付けは**未対応**（クレート跨ぎテストは定義クレート内でビジターを構築する）。
> - **リント。** `unused entry` 警告は実装済み；任意の「何も辿らない」リントは**未実装**。
