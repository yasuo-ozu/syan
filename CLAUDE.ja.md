...
`Foo`」という明確なエラーになる。これは（`#[derive(Ast)]` が既に出している）`Repeater` impl に消費者を
与える: ポータブルなサブ AST 参照。

## `#[derive(Ast)]`（`macro/ast.rs`）

- ヘルパー属性に `no_ast` を追加: `#[proc_macro_derive(Ast, attributes(syan, no_ast))]`；フィールド
  ごとに `#[no_ast]` を読む（他のヘルパー属性と同様、埋め込む def からは除去する）。
- メタデータマクロは variant → field ごとにレコードを出す: **アクセサ**（タプル添字 or 名前付き
  ident）、**コンテナ**（`direct` / `box` / `vec` / `option` / … — Decision 1 参照）、そして
  `#[no_ast]` でないフィールドには内側の**ヘッド ident** ＋ そのフィールド型のメタデータマクロへの
  **ポータブルなマクロパス**（ビジターのコンテキストから呼べるように）。`#[no_ast]` フィールドは
  `@no_ast` だけを持つ。（同等に、derive がクリーンな def を埋め込み続けて `__visitor_build` が
  フィールドごとに再導出してもよいが、明示レコードにすると container/`#[no_ast]` の分類を derive 側で
  持てる。）
- `Leaker` ＋ `Repeater<N>` impl は維持し、`Referrer` を使ってフィールドごとの型/マクロパスをポータブル
  にする（type-leak の作業を正当化する消費者）。
...
- **自動探索 ＋ 逐次生成**（*スケール* 参照）: ping-pong の各バウンスで 1 つの型の構造を取得し、*その型の*
  ...
  name-list に記録し、未発見の非 `#[no_ast]` フィールドヘッド型をキューに積み、構造は捨てる。
- **本体のローワリング**（フィールドごと；コンテナを剥がして内側ヘッドを visit）:
  - `#[no_ast]` ⇒ `_` に束縛してスキップ。
  - それ以外 ⇒ `this.visit_<inner-head>(<access expr>)`。access expr はコンテナのローワリングを適用。
    全ての内側ヘッドはメソッドを持つ発見済みの型なので、**メンバシップ判定は不要**。
...
### パス解決。
発見したサブ型のマクロ/型パス ＝ 書かれたとおりのフィールド型のパス。相対のときは*定義元*の型のモジュール
を前置して絶対化する（`Expr @ super::ast` 中の `Stmt` ⇒ `super::ast::Stmt`）；明示パス
（`crate::other::Stmt`）はそのまま使う。import 相対のパス（`use other::Stmt; … Stmt`）が残る穴で、
`Leaker`/`Repeater` ＋ `$crate` の仕組みがそれを塞ぐ（正準な同一性 / ポータブルなパス）。
...
- `#[no_ast]`: リーフ/リーフのコンテナのフィールドはスキップ；`#[no_ast]` の無い非 Ast フィールドは明確な
  「cannot find macro」エラー。
